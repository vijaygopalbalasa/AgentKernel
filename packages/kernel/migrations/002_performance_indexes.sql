...
CREATE INDEX IF NOT EXISTS idx_agents_state_last_active
...
CREATE INDEX IF NOT EXISTS idx_episodic_agent_created
    ON episodic_memories(agent_id, created_at DESC);

-- Episodic memories: importance-weighted retrieval per agent
CREATE INDEX IF NOT EXISTS idx_episodic_agent_importance
    ON episodic_memories(agent_id, importance DESC);

-- Capability tokens: validity lookups (permission checks at request time)
CREATE INDEX IF NOT EXISTS idx_capability_tokens_agent_valid
    ON capability_tokens(agent_id, valid_until)
    WHERE revoked_at IS NULL;

-- Agent permissions: permission lookups by agent (authorization checks)
CREATE INDEX IF NOT EXISTS idx_agent_permissions_agent_expires
