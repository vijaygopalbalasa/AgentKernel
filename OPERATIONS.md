# AgentKernel Operational Playbooks

This document provides operational runbooks for self‑hosted AgentKernel deployments.
...
AgentKernel can run agents in isolated Docker containers (recommended for production hardening).
...
- Optional: `AGENT_WORKER_DOCKER_MOUNT=/absolute/path/to/AgentKernel` to mount host artifacts.
- Optional hardening:
...
AgentKernel supports distributed job execution via Postgres advisory locks.
...
## Memory Archival
AgentKernel can archive memory rows into `memory_archives` before deletion.
...
## Memory Encryption
AgentKernel supports optional at-rest encryption for memory payloads.
...
AgentKernel supports a lightweight leader election using PostgreSQL advisory locks.
